...
public class HTMLTagBalancer implements XMLDocumentFilter, HTMLComponent {
    ...
    /** Recognized features. */
    private static final String[] RECOGNIZED_FEATURES = { NAMESPACES, AUGMENTATIONS, REPORT_ERRORS, DOCUMENT_FRAGMENT_DEPRECATED,
    ...
    public void startElement(final QName elem, XMLAttributes attrs, final Augmentations augs) throws XNIException {
        ...
        if ((fElementStack.top > 1 && (fElementStack.peek().element.code == HTMLElements.SCRIPT)) || fElementStack.top > 2
    ...
    public void emptyElement(final QName element, final XMLAttributes attrs, final Augmentations augs) throws XNIException {
        ...
        if (elem.isEmpty() || fAllowSelfclosingTags || elem.code == HTMLElements.UNKNOWN
    ...
    protected final int getElementDepth(final HTMLElements.Element element) {
        ...
        final short elementCode = element.code;
        final boolean tableBodyOrHtml =
                (elementCode == HTMLElements.TABLE) || (elementCode == HTMLElements.BODY) || (elementCode == HTMLElements.HTML);
        int depth = -1;
        for (int i = fElementStack.top - 1; i >= fragmentContextStackSize_; i--) {
            final Info info = fElementStack.data[i];
            if (info.element.code == element.code
                    && (elementCode != HTMLElements.UNKNOWN || (elementCode == HTMLElements.UNKNOWN && element.name
                            .equals(info.element.name)))) {
